  ...
  return (
    <Modal open={open} title={editing ? "编辑连接" : "新建连接"} width={840} onClose={close} footer={<><button type="button" onClick={close}>取消</button><button className="primary-button" type="button" onClick={() => void save()}>确定</button></>}>
      <div className="connection-editor">
        ...
        <section>
          {section === "general" && <><fieldset><legend>常规</legend><div className="form-grid"><label className="wide">名称<input autoFocus value={form.name} onChange={(event) => update("name", event.target.value)} /></label><label className="wide">目录<select value={form.folderId ?? ""} onChange={(event) => update("folderId", event.target.value || null)}><option value="">根目录</option>{folders.map((folder) => <option key={folder.id} value={folder.id}>{folder.name}</option>)}</select></label><label className="host-field">主机<input value={form.host} onChange={(event) => update("host", event.target.value)} /></label><label>端口<input type="number" min={1} max={65535} value={form.port} onChange={(event) => update("port", Number(event.target.value))} /></label></div></fieldset><fieldset><legend>认证</legend><div className="form-grid"><label>方法<select value={form.authMethod} onChange={(event) => update("authMethod", event.target.value as AuthMethod)}><option value="password">密码</option><option value="public_key">公钥</option></select></label><span /><label>用户名<input value={form.username} onChange={(event) => update("username", event.target.value)} /></label><span />{form.authMethod === "password" ? <label className="wide">密码<input type="password" placeholder={editing ? "留空则保持原密码" : ""} value={form.password ?? ""} onChange={(event) => update("password", event.target.value)} /></label> : <label className="wide">私钥<select value={form.keyId ?? ""} onChange={(event) => update("keyId", event.target.value || null)}><option value="">选择私钥</option>{keys.map((key) => <option key={key.id} value={key.id}>{key.name} ({key.algorithm})</option>)}</select></label>}</div></fieldset></>}
