...
import type { ProxyProfile, RouteCandidate, RouteKind, RouteProfile, VaultStatus } from "../../types";
    ...
    <>
      <Modal open={open} title="设置" width={860} onClose={() => setOpen(false)} footer={<button type="button" onClick={() => setOpen(false)}>关闭</button>}>
        <div className="settings-layout">
          ...
          <section>
            {section === "general" && <div className="settings-section"><h2>常规</h2><label>下载目录<input value="程序目录/downloads" readOnly /></label><label>界面语言<select defaultValue="zh-CN"><option value="zh-CN">简体中文</option></select></label><label className="checkbox-line"><input type="checkbox" defaultChecked />关闭窗口前确认活动会话</label><p>运行数据、下载文件和日志均保存在 FunShell.exe 所在目录。</p></div>}
