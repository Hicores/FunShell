...
- `russh-sftp 2.3` deserializes directory metadata with numeric UID/GID but usually leaves `user/group` names empty. Resolve those IDs directly with remote `getent` lookups (falling back to `/etc/passwd` and `/etc/group`) instead of parsing server-specific `stat` output, and retain numeric IDs as the fallback for file listings and permission editing.
