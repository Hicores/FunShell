...
- `russh-sftp 2.3` deserializes directory metadata with numeric UID/GID but usually leaves `user/group` names empty. Resolve names with batched remote `stat` and `getent` lookups, and retain numeric IDs as the fallback for file listings and permission editing.
