...
- `russh-sftp 2.3` deserializes directory metadata with numeric UID/GID but usually leaves `user/group` names empty. Resolve names with a batched remote `stat` lookup and retain numeric IDs as the fallback for file listings and permission editing.
