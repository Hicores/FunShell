...
- Register `tauri-plugin-single-instance` before every other Tauri plugin. Its callback must show, unminimize, and focus the existing `main` window so repeated launches restore the current workspace instead of initializing another process.
